...
| # | Label as shown | File:line | Domain function | Payments fields / formula | Independent recomputation | Verdict | Evidence |
|---|---|---|---|---|---|---|---|
| 52 | Oxygen pill text +Nd / Nd / … | `AppShell.tsx:33–34,80` | computeOxygen (oxygen.ts) | Σ per closing: exit date before − after, at that day's ledger and pace [file_cases.closing_date ?? notes.start_date, netProfit] | O1: 533; O2: 533; O5: 0 | FLAG (F12, F20) | O1✓ O2✓ O5✓ OXY7✓ |
| 53 | aria-label: Oxygen {oxygenLabel} / Oxygen loading | `AppShell.tsx:76` | computeOxygen (oxygen.ts) | Σ per closing: exit date before − after, at that day's ledger and pace [file_cases.closing_date ?? notes.start_date, netProfit] | O1: 533; O2: 533; O5: 0 | FLAG (F12, F20) | O1✓ O2✓ O5✓ OXY7✓ |
...
| # | Label as shown | File:line | Domain function | Payments fields / formula | Independent recomputation | Verdict | Evidence |
|---|---|---|---|---|---|---|---|
| 66 | Main score N + days | `OxygenScore.tsx:36–37` | computeOxygen (oxygen.ts) | Σ per closing: exit date before − after, at that day's ledger and pace [file_cases.closing_date ?? notes.start_date, netProfit] | O1: 533; O2: 533; O5: 0 | FLAG (F12, F20) | O1✓ O2✓ O5✓ OXY7✓ |
...
Lamar costs $484,000 vs capital $475,000; Wichita $1,217,000 vs $1,197,000. The app uses `investor_capital` as the capital basis (documented) and land cost per lot from that basis, so the extra costs are not in any figure. The gap is recent: the pinned fixture (2026-09-11 21:31 UTC) still had `investor_capital` **equal** to the costs on both farms (484,000 / 1,217,000) and the live page read capital owed $4,145,355.48 (OPEN_QUESTIONS #105); by the 2026-09-12 04:30 UTC snapshot both rows had been reduced by exactly the $9,000 / $20,000, moving capital owed to $4,116,355.48 and raising gross on those farms by the same $29,000 (Lamar +$1,000 per lot, Wichita +$625 per lot). It also flipped a state, not just a sum: Lamar has exactly $475,000 of `capital_return` distributions, so at $484,000 it was a captive position with $9,000 outstanding (`fixture.test.ts` still asserts precisely that on the pinned fixture) and at $475,000 it is **freed** — the only freed position, the *Debt of Honor* trophy, the Liberation board's “walked free” line and the only completed rotation cycle (271 d, pre-era) all exist today because of that overnight edit. `farm_acquisitions` has no `updated_at`, so who changed it and why is not visible to Quest. Query: `select fa.farm_name, fa.investor_capital, sum(pc.amount) from farm_acquisitions fa join property_costs pc on pc.farm_acquisition_id = fa.id group by 1,2 having sum(pc.amount) <> fa.investor_capital;`
...
### F20 · MODEL DECISION · The cumulative Oxygen score (`totalDaysGained`) is a sum of non-commensurable units and has no valid interpretation as a total

`oxygen.ts` `totalDaysGained = Σ daysGained` over every closed lot, where each `daysGained_i = netProfit_i ÷ pace(d_i)` and `pace(d_i)` is the realm's net profit per day *on that lot's own closing day*. Each term is a shift of the projected exit date — but of a different projection: the one current on day d_i, with that day's remaining and that day's pace. Shifts of different projections do not add up to the shift of any projection, so the 533 of the live snapshot is 38 numbers in 38 different units that happen to share the word "days". There is no sentence a user can say about it: it is not "533 days ahead" of the deadline, of the original plan, or of any date. The spread of denominators is not academic — the paces behind the 38 terms run from $339/day (Lamar Lot 6, 2025-10-10, +90 days; a lone closing in its window, see F19) to $9,168.78/day today (O3), a factor of 27, so one early closing contributes 17 % of the total on its own. Nothing here is arithmetically wrong: every per-lot figure is exactly what the code documents, the independent replay reproduces the total (O2 = 533), and the ledger's Σ equals the card's figure (e2e). This is a finding about what the sum *means*, not about how it is computed — hence a model decision, not a bug.

Two figures the card already computed are interpretable and were buried. `trailingDaysGained` (53) sums only the closings inside the trailing window, and the window itself (`goal.trailingWindowDays`, 90) is a quantity on the same clock: gaining fewer days than passed means the closings of the window pulled the exit date closer by less than the window moved it away, i.e. at this pace the exit date drifts away by 37 days every 90; more than passed means it is being pulled closer. Caveat stated for the record: the trailing figure still mixes paces within the window (they vary far less over 90 days than over the history), and it counts the closings' contribution only — pace drift between closings also moves the projected date and is in neither figure. The card was restructured on that comparison (PR [#10](https://github.com/terrafunded/terrafunded-quest-v2/pull/10), "Oxygen — the honest scoreboard" in PROGRESS.md): headline 53 *days gained in the last 90* with the explicit arithmetic and a colour by band; the cumulative total demoted to a small line labelled "Cumulative historical score" with a tooltip on the per-lot method; the topbar pill switched to the trailing reading (`53/90d`). Rows 52, 53 and 66 above describe the pre-redesign layout and remain correct as arithmetic.

**Alternatives considered for the per-lot figures themselves:**

(a) *Restate every closing at today's pace.* `daysGained_i = netProfit_i ÷ pace(today)`; the total becomes `Σ netProfit_i ÷ pace(today) = netProfitToDate ÷ pace(today)` = $2,242,037.41 ÷ $9,168.78 ≈ 244.5 days, one denominator, a valid reading ("at today's pace, everything closed so far is worth 244 days of the realm's output"). Costs: it is the net-profit counter divided by a constant, so it says nothing the headline dollar figure does not; it changes every day as the pace moves and rewrites every past ledger row, chronicle line, "latest breath" and "deepest breath" retroactively — a fast realm today would shrink every old deal, a slow week would inflate them; it destroys the one property that makes the per-lot number a fact about the deal ("what this closing did to the exit date the day it happened"); and it breaks the `narrative.ts` / `story.ts` / Quests ledger consumers that quote per-lot days as history.

(b) *Keep the frozen-at-close method (retained).* Each lot keeps `netProfit_i ÷ pace(d_i)`, measured on the ledger as it stood that day: individually correct, reproducible from the rows (O2; with the F12 and F19 caveats on today's net values and out-of-order entry), stable as the realm speeds up, and the right unit for the chronicle, the Quests ledger, the latest/deepest cards and the trailing-window comparison, which needs each closing scored on the projection current when it happened. The only thing lost is the meaning of the lifetime sum — and that is a presentation problem, solved by not leading with it.

**Recommended action:** No change to `computeOxygen`: the frozen-at-close per-lot figures stay as defined. Treat `totalDaysGained` as a historical tally (the game's score that only goes up), never as a distance to anything, and never compare it to a number of calendar days; the card, the pill and the copy now do this. If a lifetime figure in commensurable units is ever wanted, use (a) as an *additional* derived figure labelled "at today's pace", not as a replacement. Done in PR #10 at the presentation layer; the domain is unchanged.

**Who decides:** Product (whether the cumulative tally should stay on the card at all); engineering done.
