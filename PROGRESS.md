...
| | |
|---|---|
| **URL** | **https://terrafunded-quest-v2.vercel.app** — *prepared, not yet live*: the Cloud Agent has no Vercel credential (only the four Supabase/viewer secrets are injected), so the deploy is one `npm run deploy` away. **`DEPLOY.md`** has the two-minute laptop path and the one-secret (`VERCEL_TOKEN`) path that lets the agent do it. |
...
| **Platform** | `vercel.json`: Vite framework preset, `npm run build` → `dist/`, every route rewritten to `index.html` (SPA deep links), immutable cache on `/assets`, nosniff / frame-deny / referrer headers. No server, no API routes. |
