...
1. Build the realm at each of the three exit horizons against the same snapshot.
2. Derive Engine + Council the same way the pages do.
3. Extract every displayed figure with its label and classification.
4. Assert historical identity, horizon-dependent direction, and labeled deliberately-independent figures.

## Violations found and fixed

| id | class | bug | fix |
|---|---|---|---|
| `/engine.shortfallDollars` | HORIZON-DEPENDENT | Plateaued at $0 for 2028 and 2029 under a strict `decrease` rule | Direction `non_increase` (floor at $0 is correct once the longer horizon clears the goal) |
| `/warplan.rotationFarms` | HORIZON-DEPENDENT | 5 → 5 → 4 under strict `decrease` | Direction `non_increase` (ceiling / discrete farms) |
| `/council.pace.*` required keys | HISTORICAL (misclassified) | `requiredClosingsPerMonth` / `daysLeft` moved with the horizon | Reclassified as HORIZON-DEPENDENT |
| `/council.conversion.required*` | HISTORICAL (misclassified) | Required pace moved with the horizon | Reclassified as HORIZON-DEPENDENT |
| `/council.*.severity` (stuck, inventory, concentration, …) | HORIZON-DEPENDENT (misclassified) | Book-reading severities that never consult the deadline | Reclassified as HISTORICAL |
| `/council.pace.severity` / `conversion.severity` | HORIZON-DEPENDENT | Softened with horizon but not all-three-distinct | Severity rank with `non_increase` |
| `/council.inventory.needed` / `inventoryGap` | DELIBERATELY INDEPENDENT unlabeled | Flat closings-to-goal next to moving figures | On-screen note: remaining ÷ avg $/lot, same at every exit year |
| `/trophies.count` | HISTORICAL | Pace Keeper flipped on 2029 | Split: historical earned (ex Pace Keeper) + `paceKeeperEarned` HORIZON-DEPENDENT; card note |
| Throne vs Engine farms | dual source | Could drift apart | Both read `sharedFarmsStillNeeded` / `farmsStillNeededWithTurns`; Engine shows the shared figure beside fresh-capital shortfall |

Permanent guard: `src/domain/__tests__/horizonFigures.test.ts` — captures all three horizons and asserts the invariants above.
...
| `/engine.netProfitNoFresh` | /engine | Net profit without fresh capital | 6440143.65 | 10066914.65 | 13770532.34 | HORIZON-DEPENDENT | increase |
| `/engine.peakOutstanding` | /engine | Peak outstanding | 3995739.78 | 3995739.78 | 3995739.78 | DELIBERATELY INDEPENDENT | labeled |
| `/engine.reservedLots` | /engine | Reserved lots | 33 | 33 | 33 | HISTORICAL |  |
| `/engine.shortfallDollars` | /engine | Shortfall $ | 3559856.35 | 0 | 0 | HORIZON-DEPENDENT | non_increase |
| `/engine.totalInterest` | /engine | Total interest to deadline | 766251.76 | 1288950.9 | 1808632.22 | HORIZON-DEPENDENT | increase |
| `/engine.turns` | /engine | Capital turns | 0.62 | 1.25 | 1.87 | HORIZON-DEPENDENT | increase |
...
### Live PASS/FAIL (Playwright against production, 2026-09-14 ≈23:58 UTC)
...
| 6 | Usable at 380px | **PASS** | verdict width 312px; screenshots `engine-mobile-380.png` |
| 7 | Throne Room | **PASS** | `/` net-profit-counter |
| 8 | The Realm | **PASS** | `/realm` |
| 9 | Sponsors | **PASS** | `/sponsors` |
| 10 | Council | **PASS** | `/council` |
| 11 | War Plan | **PASS** | `/warplan` inputs |

Screenshots: `/opt/cursor/artifacts/screenshots/engine-*.png`, `throne.png`, `realm.png`, `sponsors.png`, `council.png`, `warplan.png`.
...
| **Platform** | `vercel.json`: Vite framework preset, `npm run build` → `dist/`, SPA routes rewritten to `/index` except `/api/*` (`/((?!api/).*)`), immutable cache on `/assets`, nosniff / frame-deny / referrer headers. One serverless function: `api/weekly-council.ts` (the weekly read). |
| **Env** | `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `QUEST_ALLOWED_TEST_EMAIL` (the questbot's address, a hidden Secret), and `ANTHROPIC_API_KEY` (server-only, never `VITE_`, read only inside `api/weekly-council.ts`). The anon key is public by design and RLS keeps Payments read-only. |
| **Check** | `npm run verify:live -- <url>` (`scripts/verify-live.ts`): deep link `/pipeline` is not a 404, proves the Payments-staff gate on the live bundle (login footer names it; a sign-in staged as a non-admin is refused with "Quest is for the TerraFunded team only." and left with no session), signs in as the allowed questbot and read the trapped-profit counter and the verdict, then picks each of the three themes on `/login`, asserts `html[data-theme]` + `localStorage` and that the choice survives a reload, and saves `docs/live-<theme>.jpg`. Run against the local production build (`http://localhost:4173`) it reports net profit **$2,272,304**, trapped **$1,116,863**, "You need 8.31 lots/month; you are doing 4.4." and all three skins switching — the same run is the acceptance test for the live URL. |
...
## The Weekly Council (`/council`)

Deterministic rule engine (`src/domain/council.ts`) plus an LLM-written weekly plan. The model
does not compute, does not touch Payments, and does not do arithmetic. It receives
`buildWeeklyFacts` (pre-formatted) and writes `{ week_of, headline, actions, watch_out }`.

- Eight rules, always, in fixed order: pace, stuck, inventory, concentration, losing_ground,
  conversion, recycle, quality. Concentration threshold `SPONSOR_CONCENTRATION_WARN_PCT = 33.33`
  (one third of deployed capital). Recycled capital in 90 days is read off the War Plan required
  schedule's own month grid — no new rate.
- `api/weekly-council.ts` (Node): verifies the Supabase session server-side (401 if anonymous),
  calls `claude-sonnet-4-6` (`max_tokens` 1000), validates every number in the reply against the
  payload, caches by ISO week + horizon + language. `WEEKLY_COUNCIL_REGENERATE_DAILY_LIMIT = 10`
  forced calls per user per UTC day. Missing/invalid key or a hallucinated figure → structured
  `{ unavailable: true }` so the page keeps the ledger cards and shows "weekly read unavailable".
- Cache today: `/tmp/quest-weekly-council-cache.json` (or an in-memory store in tests). Proposed
  Quest-owned tables in `sql/proposed_weekly_council.sql` — do not apply against Payments.
- `ANTHROPIC_API_KEY` is read only from `process.env` inside the function. Vite `envPrefix` does
  not include it. The client talks only to `/api/weekly-council`. The test suite greps `src/` and
  `dist/` for `sk-ant` and asserts zero matches.
