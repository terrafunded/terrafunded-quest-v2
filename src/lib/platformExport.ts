...
function collectFigures(
  ...
  return [
    ...
    {
      id: "realm.scorecardFreestoneNetPerSoldLot",
      page: "/realm",
      section: "scorecard",
      label: REALM_MAP_UI[lang].scorecard.col.netPerLot + " · Freestone",
      displayed: money(realm.farmScorecard.rows.find((r) => r.name === "Freestone")?.netProfitPerSoldLot ?? null, lang),
      raw: realm.farmScorecard.rows.find((r) => r.name === "Freestone")?.netProfitPerSoldLot ?? null,
      subtitle: realm.farmScorecard.rows.find((r) => r.name === "Freestone")?.grade ?? null,
      units: "USD/lot",
      source: { file: "src/domain/farmScorecard.ts", export: "computeFarmScorecard" },
      inputs: {
        soldLots: realm.farmScorecard.rows.find((r) => r.name === "Freestone")?.soldLots ?? 0,
        netProfit: realm.farms.find((f) => f.name === "Freestone")?.netProfit ?? 0,
      },
      formula: "farm.netProfit ÷ soldLots; grade from net/lot + median funding→close (AUDIT.md §7)",
    },
    {
      id: "realm.scorecardWichitaNetPerSoldLot",
      page: "/realm",
      section: "scorecard",
      label: REALM_MAP_UI[lang].scorecard.col.netPerLot + " · Wichita",
      displayed: money(realm.farmScorecard.rows.find((r) => r.name === "Wichita")?.netProfitPerSoldLot ?? null, lang),
      raw: realm.farmScorecard.rows.find((r) => r.name === "Wichita")?.netProfitPerSoldLot ?? null,
      subtitle: realm.farmScorecard.rows.find((r) => r.name === "Wichita")?.grade ?? null,
      units: "USD/lot",
      source: { file: "src/domain/farmScorecard.ts", export: "computeFarmScorecard" },
      inputs: {
        soldLots: realm.farmScorecard.rows.find((r) => r.name === "Wichita")?.soldLots ?? 0,
      },
      formula: "farm.netProfit ÷ soldLots",
    },
    {
      id: "realm.scorecardLakeviewSold",
      page: "/realm",
      section: "scorecard",
      label: REALM_MAP_UI[lang].scorecard.col.lots + " · Lakeview",
      displayed: `${realm.farmScorecard.rows.find((r) => r.name === "Lakeview")?.soldLots ?? 0} / ${realm.farmScorecard.rows.find((r) => r.name === "Lakeview")?.totalLots ?? 0}`,
      raw: realm.farmScorecard.rows.find((r) => r.name === "Lakeview")?.soldLots ?? 0,
      subtitle: null,
      units: "lots",
      source: { file: "src/domain/farmScorecard.ts", export: "computeFarmScorecard" },
      inputs: { totalLots: realm.farmScorecard.rows.find((r) => r.name === "Lakeview")?.totalLots ?? 0 },
      formula: "soldLots of the Lakeview farm row",
    },
    {
      id: "realm.scorecardStaleCount",
      page: "/realm",
      section: "scorecard",
      label: REALM_MAP_UI[lang].scorecard.staleFlag,
      displayed: fmtNumber(realm.farmScorecard.staleCount, lang),
      raw: realm.farmScorecard.staleCount,
      subtitle: null,
      units: "farms",
      source: { file: "src/domain/farmScorecard.ts", export: "computeFarmScorecard" },
      inputs: { staleAfterDays: realm.farmScorecard.staleAfterDays },
      formula: "count of subdivided farms with zero reservation events in the last 90 days",
    },
    {
      id: "treasury.liquidityCostRealized",
      page: "/treasury",
      section: "notes",
      label: tr.notes.realized,
      displayed: money(realm.noteStrategies.liquidityCostRealized, lang),
      raw: realm.noteStrategies.liquidityCostRealized,
      subtitle: null,
      units: "USD",
      source: { file: "src/domain/profitLayers.ts", export: "computeProfitLayers" },
      inputs: { notesSoldCount: realm.profitLayers.notesSoldCount },
      formula: "Σ (noteFinancedAmount − noteSalePrice) over sold notes",
    },
    {
      id: "treasury.liquidityCostUnrealized",
      page: "/treasury",
      section: "notes",
      label: tr.notes.unrealized,
      displayed: money(realm.noteStrategies.liquidityCostUnrealized, lang),
      raw: realm.noteStrategies.liquidityCostUnrealized,
      subtitle: `${(realm.noteStrategies.noteSaleRatio * 100).toFixed(2)}%`,
      units: "USD",
      source: { file: "src/domain/profitLayers.ts", export: "computeProfitLayers" },
      inputs: {
        notesHeldFace: realm.noteStrategies.notesHeldFace,
        noteSaleRatio: realm.noteStrategies.noteSaleRatio,
      },
      formula: "Σ noteFinancedAmount × (1 − measured note-sale ratio) over held notes",
    },
    {
      id: "treasury.liquidityCostTotal",
      page: "/treasury",
      section: "notes",
      label: tr.notes.totalCost,
      displayed: money(realm.noteStrategies.liquidityCostTotal, lang),
      raw: realm.noteStrategies.liquidityCostTotal,
      subtitle: tr.notes.totalIdentity(
        money(realm.noteStrategies.liquidityCostTotal, lang),
        money(realm.noteStrategies.liquidityCostRealized, lang),
        money(realm.noteStrategies.liquidityCostUnrealized, lang),
      ),
      units: "USD",
      source: { file: "src/domain/noteStrategies.ts", export: "computeNoteStrategies" },
      inputs: {
        realized: realm.noteStrategies.liquidityCostRealized,
        unrealized: realm.noteStrategies.liquidityCostUnrealized,
      },
      formula: "liquidityCostRealized + liquidityCostUnrealized",
    },
    {
      id: "treasury.sellNowCashInHand",
      page: "/treasury",
      section: "notes",
      label: `${tr.notes.sell} · ${tr.notes.cashNow}`,
      displayed: money(realm.noteStrategies.sell.cashInHandNow, lang),
      raw: realm.noteStrategies.sell.cashInHandNow,
      subtitle: null,
      units: "USD",
      source: { file: "src/domain/noteStrategies.ts", export: "computeNoteStrategies" },
      inputs: {
        cashAlreadyRealized: realm.noteStrategies.cashAlreadyRealized,
        notesHeldAtRatio: realm.noteStrategies.notesHeldAtRatio,
      },
      formula: "(cashAtClosing + noteSaleProceeds) + notesHeldFace × measured ratio",
    },
    {
      id: "treasury.holdTotalValue",
      page: "/treasury",
      section: "notes",
      label: `${tr.notes.hold} · ${tr.notes.totalValue}`,
      displayed: money(realm.noteStrategies.hold.totalValue, lang),
      raw: realm.noteStrategies.hold.totalValue,
      subtitle: null,
      units: "USD",
      source: { file: "src/domain/noteStrategies.ts", export: "computeNoteStrategies" },
      inputs: {
      ...
      formula: "cashAtClosing + noteSaleProceeds + notesHeldFace (no invented collections)",
    },
    {
      id: "treasury.deliverLpReturnable",
      page: "/treasury",
      section: "notes",
      label: `${tr.notes.deliver} · ${tr.notes.lpReturnable}`,
      displayed: money(realm.noteStrategies.deliver.lpCapitalReturnable, lang),
      raw: realm.noteStrategies.deliver.lpCapitalReturnable,
      subtitle: tr.notes.footnote,
      units: "USD",
      source: { file: "src/domain/noteStrategies.ts", export: "computeNoteStrategies" },
      inputs: {
        lpCapitalOwed: realm.noteStrategies.lpCapitalOwed,
        cashAlreadyRealized: realm.noteStrategies.cashAlreadyRealized,
        notesHeldFace: realm.noteStrategies.notesHeldFace,
      },
      formula: "min(debt.capitalOwed, cashAtClosing + noteSaleProceeds + notesHeldFace)",
    },
    {
      id: "pipeline.aging90Plus",
      page: "/pipeline",
      section: "aging",
      label: pipe.aging.bucket90,
      displayed: fmtNumber(realm.reservationAging.stuckCount, lang),
      raw: realm.reservationAging.stuckCount,
      subtitle: money(realm.reservationAging.stuckExpectedNet, lang),
      units: "reservations",
      source: { file: "src/domain/reservationAging.ts", export: "computeReservationAging" },
      inputs: {
        stuckSalePrice: realm.reservationAging.stuckSalePrice,
        stuckExpectedNet: realm.reservationAging.stuckExpectedNet,
      },
      formula: "live reservations with daysWaiting ≥ 90",
    },
    {
      id: "pipeline.fullyReservedUnsold",
      page: "/pipeline",
      section: "aging",
      label: pipe.aging.fullyReserved,
      displayed: fmtNumber(realm.reservationAging.fullyReservedUnsold.length, lang),
      raw: realm.reservationAging.fullyReservedUnsold.length,
      subtitle: realm.reservationAging.fullyReservedUnsold.map((f) => f.farmName).join(", ") || null,
      units: "farms",
      source: { file: "src/domain/reservationAging.ts", export: "computeReservationAging" },
      inputs: {},
      formula: "farms where reserved === totalLots and soldLots === 0",
    },
    {
      id: "throne.scorecardGradeA",
      page: "/",
      section: "scorecard",
      label: t.scorecardTitle,
      displayed: fmtNumber(realm.farmScorecard.gradeACount, lang),
      raw: realm.farmScorecard.gradeACount,
      subtitle: t.scorecardHint(realm.farmScorecard.gradeACount, realm.farmScorecard.staleCount),
      units: "farms",
      source: { file: "src/domain/farmScorecard.ts", export: "computeFarmScorecard" },
      inputs: { staleCount: realm.farmScorecard.staleCount },
      formula: "count of subdivided farms graded A (AUDIT.md §7)",
    },
    {
      id: "throne.parked90Plus",
      page: "/",
      section: "parked",
      label: t.parkedTitle,
      displayed: fmtNumber(realm.reservationAging.stuckCount, lang),
      raw: realm.reservationAging.stuckCount,
      subtitle: t.parkedHint(realm.reservationAging.stuckCount, money(realm.reservationAging.stuckExpectedNet, lang)),
      units: "reservations",
      source: { file: "src/domain/reservationAging.ts", export: "computeReservationAging" },
      inputs: { stuckExpectedNet: realm.reservationAging.stuckExpectedNet },
      formula: "live reservations with daysWaiting ≥ 90",
    },
    {
      id: "sponsors.count",
...
function runReconciliations(
  ...
  return [
    ...
    mkCheck(
      "note_liquidity_cost_identity",
      lang === "es"
        ? "costo de liquidez total == realizado + no realizado"
        : "total liquidity cost == realized + unrealized",
      { label: "profitLayers.liquidityCostTotal", value: layers.liquidityCostTotal },
      { label: "realized + unrealized", value: round2(layers.liquidityCostRealized + layers.liquidityCostUnrealized) },
      TOL_CENTS,
      lang === "es"
        ? "El costo de liquidez total no es la suma de realizado y no realizado."
        : "Total liquidity cost is not realized plus unrealized.",
    ),
    mkCheck(
      "goal_identity",
