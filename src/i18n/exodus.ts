...
export const EXODUS_UI: Record<QualityLang, ExodusUiStrings> = {
  en: {
    title: "Exodus",
    subtitle: (lp) =>
      `Return the ${lp} the limited partners of Portafolio Diversificado Alpha LP invested, by the deadline: a chosen share in fractions of promissory notes at 100% of their unpaid balance, the rest in cash. Production is the War Plan's required plan in cash mode; every input starts at the real figure.`,
    reset: "Reset to real data",
    verdict: "The verdict",
    verdictFoot: (months, deadline, closings, farms, ads) => `${months} months to ${deadline}. Production is the War Plan's required plan: ${closings} closings per month, ${farms} new ${farms === 1 ? "farm" : "farms"}, ${ads} of ad spend per full month.`,
    ...
    basis: { combined: "combined", financed: "price ÷ financed", oracle: "Oracle default" },
    ...
    startingCashReal: (kept, owed) => `${kept} kept by the fund so far, ${owed} owed to sponsors today (War Plan) — shown, not assumed`,
    inherited: "Pace, farm cost, lags, conversion, funding mix, cycle and seasonality are the War Plan's current inputs. Change them there.",
    ...
    inventoryFoot:
      "Active, unsold, non-test notes. Own-capital farms (Promised Valley, Olney, Red River 1) deliver at once; fixed-interest lots deliver once their ledger balance is paid by a partial release, whose cost today is the lot's outstanding balance in Payments' lot ledger and grows every month; profit-share farms (Townson) never deliver. Notes without a farm on record cannot be classified and take no part in the plan.",
    ...
    sourcesTitle: "Production, cash and what differs from the War Plan",
    productionLine: (lots, farms, ads, match) => `Production replays the War Plan's required plan: ${lots} lots closed, ${farms} farms, ${ads} of ad spend — ${match ? "every month's closings match the War Plan's row" : "some months differ from the War Plan's rows"}.`,
    cashLine: (wp, baseline) => `Cash is not the same number by design. The War Plan's cash mode lands at ${wp}; paying 100% in cash the Exodus pays ${baseline} to the LPs. The bridge:`,
    bridgeStart: "Starting position: the Exodus starts from the starting cash above, the War Plan from cash kept − owed today",
    bridgeReceipts: "Receipts: real sale ratio on the amortized balance instead of the Oracle's note-sale percent of the face value",
    bridgeExisting: "Today's notes sold, net of the partners' share (the War Plan does not model them)",
    ...
    bridgeAds: "Ad spend paid (the War Plan shows it without deducting it)",
    bridgeCarry: "Cash still negative at the deadline, never paid",
    bridgeDrift: "Replay drift (the War Plan's rows are rounded)",
    bridgeResidual: "Unexplained",
    futureNoteLine: (face, rate, term, notes) => `Every future note is given the Oracle's averages: ${face} of face value (sale price − down payment), ${rate} and ${term} months (means of ${notes} farm notes), amortized monthly.`,
  ...
  es: {
    ...
    basis: { combined: "combinada", financed: "precio ÷ financiado", oracle: "valor del Oráculo" },
    ...
    startingCashReal: (kept, owed) => `${kept} retenidos por el fondo hasta hoy, ${owed} adeudados a los sponsors hoy (War Plan) — se muestran, no se suponen`,
    inherited: "Ritmo, costo de finca, plazos, conversión, mezcla de inversionistas, ciclo y estacionalidad son los datos actuales del War Plan. Cámbielos allí.",
    ...
    colReleases: "Liberaciones parciales",
    ...
    by: (d) => `Al ${d}`,
    monthsFoot:
      "La primera fila va de hoy al fin de mes, así que sus cierres están prorrateados. Cierres, fincas nuevas y publicidad son el plan requerido del War Plan mes por mes. Cada mes: los cierres y sus enganches, luego la venta de cada pagaré no reservado para entrega, luego las cascadas por finca (interés fijo al socio hasta liberar el lote, participación al capital del sponsor y luego 50/50, capital propio a Portafolio), luego la publicidad, luego el efectivo de Portafolio compra liquidación por la mayor liquidación por dólar, y lo que queda se paga a los LP. Los pagarés libres se entregan el mes en que aparecen; alcanzada la meta en pagarés, todo pagaré adicional se vende.",
    ...
    fromExistingReleased: "Pagarés de hoy tras una liberación parcial",
    fromProjectedFree: "Pagarés futuros de fincas de capital propio",
    fromProjectedReleased: "Pagarés futuros tras una liberación parcial",
    ...
    sourcesTitle: "Producción, efectivo y qué difiere del War Plan",
    productionLine: (lots, farms, ads, match) => `La producción repite el plan requerido del War Plan: ${lots} lotes cerrados, ${farms} fincas, ${ads} de publicidad — ${match ? "los cierres de cada mes coinciden con la fila del War Plan" : "algunos meses difieren de las filas del War Plan"}.`,
    cashLine: (wp, baseline) => `El efectivo no es la misma cifra por diseño. El modo efectivo del War Plan llega a ${wp}; pagando 100% en efectivo el Exodus paga ${baseline} a los LP. El puente:`,
    bridgeStart: "Posición inicial: el Exodus parte del efectivo inicial de arriba, el War Plan de efectivo retenido − adeudado hoy",
    bridgeReceipts: "Ingresos: razón de venta real sobre el saldo amortizado en vez del porcentaje de venta del Oráculo sobre el valor nominal",
    bridgeExisting: "Pagarés de hoy vendidos, netos de la parte de los socios (el War Plan no los modela)",
    bridgePartners: "Socios pagados lote por lote por las cascadas en vez del costo íntegro de cada finca más la participación",
    bridgeSettlement: "Liberaciones parciales y fincas con efectivo propio",
    bridgeAds: "Publicidad pagada (el War Plan la muestra sin descontarla)",
    bridgeCarry: "Efectivo aún negativo en la fecha límite, nunca pagado",
    bridgeDrift: "Deriva de la repetición (las filas del War Plan están redondeadas)",
    bridgeResidual: "Sin explicar",
    futureNoteLine: (face, rate, term, notes) => `Cada pagaré futuro recibe los promedios del Oráculo: ${face} de valor nominal (precio de venta − enganche), ${rate} y ${term} meses (medias de ${notes} pagarés de fincas), amortizado mensualmente.`,
    ...
    releasesTitle: "Liberaciones parciales en este escenario",
    closingsOf: (month) => `cierres de ${month}`,
    releasedIn: (month) => `liberado ${month}`,
