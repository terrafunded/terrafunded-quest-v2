...
const T: Record<QualityLang, Record<QualityKind, (c: Ctx) => Texts>> = {
  es: {
    ...
    parcel_geometry_mismatch: (c) => ({
      ...
      explanation: `El mapa de disponibilidad dibuja ${c.num("polygons") ?? "?"} parcelas para ${c.farm}, pero Quest tiene ${c.num("lotRows") ?? "?"} lotes en Payments${c.str("lotsWithoutPolygon") ? ` (sin parcela: ${c.str("lotsWithoutPolygon")})` : ""}${c.str("polygonsWithoutLot") ? ` (parcelas sin lote: ${c.str("polygonsWithoutLot")})` : ""}.`,
  ...
  en: {
    ...
    parcel_geometry_mismatch: (c) => ({
      ...
      explanation: `The availability map draws ${c.num("polygons") ?? "?"} parcels for ${c.farm}, but Quest has ${c.num("lotRows") ?? "?"} lots in Payments${c.str("lotsWithoutPolygon") ? ` (no parcel for ${c.str("lotsWithoutPolygon")})` : ""}${c.str("polygonsWithoutLot") ? ` (parcels without a lot: ${c.str("polygonsWithoutLot")})` : ""}.`,
