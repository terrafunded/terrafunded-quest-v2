...
export function HostageBar({ h, index = 0 }: { h: Hostage; index?: number }) {
  return (
    <motion.li
      ...
      className={cn("rounded-lg border p-3", h.freed ? "border-emerald-400/40 bg-emerald-950/30" : "border-border/60 bg-muted/30")}
      data-testid="hostage"
      data-freed={h.freed}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            {h.freed ? <Unlock className="h-4 w-4 text-emerald-300" /> : <Lock className="h-4 w-4 text-muted-foreground" />}
        ...
        <span className={cn("shrink-0 font-heading tabular", h.freed ? "text-emerald-200" : "text-foreground")}>{pct(h.pctReturned, h.pctReturned === 100 ? 0 : 1)}</span>
      </div>
      <div className="mt-2 h-2 overflow-hidden rounded-full bg-background/70">
        <motion.div
          className={cn("h-full rounded-full", h.freed ? "bg-emerald-400" : "bg-gradient-to-r from-fuchsia-400/70 to-gold")}
          initial={{ width: 0 }}
    ...
    <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
      <section className="parchment-card p-5" aria-label="Hostages of the realm">
        <div className="mb-3 flex items-baseline justify-between gap-3">
          <h2 className="font-heading text-sm uppercase tracking-[0.2em] text-fuchsia-200">
      ...
      <section className="parchment-card border-emerald-400/30 p-5" aria-label="Liberated" data-testid="liberated-gallery">
        <h2 className="mb-3 font-heading text-sm uppercase tracking-[0.2em] text-emerald-200">
