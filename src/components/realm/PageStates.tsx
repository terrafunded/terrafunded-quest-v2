...
export function PageHeader({ title, subtitle, children }: { title: string; subtitle?: ReactNode; children?: ReactNode }) {
  return (
    <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 className="font-display text-2xl text-gold sm:text-3xl">{title}</h1>
...
export function LoadingState({ rows = 6 }: { rows?: number }) {
  return (
    <div className="space-y-4" role="status" aria-live="polite" aria-label="Loading realm data">
...
export function ErrorState({ error, onRetry }: { error: Error; onRetry?: () => void }) {
  return (
    <div className="parchment-card flex flex-col items-center gap-3 border-destructive/40 px-6 py-12 text-center" role="alert">
      ...
      <h2 className="font-heading text-lg">The chronicle could not be read</h2>
  ...
  return (
    <div className="mb-4 rounded-md border border-stage-reserved/40 bg-stage-reserved/10 px-4 py-3 text-sm" role="alert">
      <div className="flex items-center gap-2 font-medium text-stage-reserved">
        <AlertTriangle className="h-4 w-4" /> Some tables could not be read; numbers below are partial.
      </div>
      <ul className="mt-1 list-inside list-disc text-xs text-muted-foreground">
        {errors.map((e) => (
          <li key={e.table}>
            <code>{e.table}</code>: {e.code ?? "error"} — {e.message}
          </li>
        ))}
      </ul>
    </div>
