...
#    the two VITE_ ones plus QUEST_ALLOWED_TEST_EMAIL (the questbot's address — the one non-admin
#    the staff-only gate admits; baked in at build time) are what the browser bundle needs. The
#    QUEST_TEST_* pair is only for the smoke test and Playwright and is NOT needed on Vercel —
#    keep it in your local .env.
...
printf '%s' "$QUEST_ALLOWED_TEST_EMAIL"  | npx vercel env add QUEST_ALLOWED_TEST_EMAIL  production
...
printf '%s' "$QUEST_ALLOWED_TEST_EMAIL"  | npx vercel env add QUEST_ALLOWED_TEST_EMAIL  preview

# 4. Deploy the v2 branch to production
npm run deploy            # = npx vercel deploy --prod --yes   → prints the live URL

# 5. Verify (proves the staff-only gate refuses a staged non-admin, signs in as the allowed
#    questbot, checks the Throne Room shows real numbers, deep links work, all three themes switch
#    and persist; writes docs/live-<theme>.jpg)
cp .env.example .env && $EDITOR .env      # fill the five values once
...
printf '%s' "$QUEST_ALLOWED_TEST_EMAIL" | npx -y vercel@latest env add QUEST_ALLOWED_TEST_EMAIL production --token "$VERCEL_TOKEN"
...
| `QUEST_TEST_EMAIL` | local `.env` only (smoke test, Playwright) | the viewer login (the questbot) |
| `QUEST_TEST_PASSWORD` | local `.env` only | its password |
| `QUEST_ALLOWED_TEST_EMAIL` | Vercel production + preview; local `.env` | the same address as `QUEST_TEST_EMAIL`. Baked into the bundle at **build** time (Vite `envPrefix` admits `QUEST_ALLOWED_*`, nothing else outside `VITE_*`): the one login without `profiles.role = 'admin'` that Quest lets in, so the smoke test and Playwright keep working. Set on 2026-09-11 for both environments (`vercel env ls` shows it as a hidden Secret). Change it → redeploy |
...
**Who can enter.** Quest is for the TerraFunded team: after sign-in it reads the user's own
`profiles` row (`id, role`) and admits only `role = 'admin'`; any other role (`investor`, `viewer`,
`note_buyer`) is signed out at once with "Quest is for the TerraFunded team only." The allow-list
above is the single exception. There is nothing to configure on Vercel for this beyond the
variable; the roles live in Payments.

## Checklist after the first deploy

- [ ] `npm run verify:live -- <url>` passes (prints the access-gate line, net profit, trapped profit, three theme lines).
